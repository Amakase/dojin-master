...
class AiBoothPlacementJob < ApplicationJob
  ...
  def perform(event_id)
    ...
    unplaced.each_slice(BATCH_SIZE) do |booths|
      response = call_llm(attachment, booths)
      ...
      EventMapCoordinate.insert_all(rows, on_duplicate: :skip)
    end
    ...
    broadcast_status(event, "failed", "AI placement failed...")
  end
  ...
  def call_llm(attachment, batch)
    # - Build a chat via RubyLLM.chat(model: "gpt-4o")
    chat = RubyLLM.chat(model: 'gpt-4o')
    # - Send the base64 image + build_prompt(batch)
    JSON.parse(chat.ask(build_prompt(batch), with: attachment).content)
    # - Parse and return the JSON array from the response
  ...
  def build_prompt(batch)
    <<~PROMPT
      ...
      Locate the following booth spaces: #{batch.to_json}
      ...
      If you cannot confidently locate a booth space, omit it from the array entirely.
      If you cannot locate any booth spaces, return an empty array: []
    PROMPT
  end
